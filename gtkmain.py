    ...
    if tweet:
        ...
        if body_container:
            ...
            if tweet_info:
                ...
                #print(match.group(1))
                return match.group(1), tweet_author_name.text
...
def ai_setumei_comment(text):
  ...
  #print("res画像",res)
  text = res.replace(':', '：') 
  prompt1_match = re.search(r"画像1のプロンプト：(.*?)(?:\n\n|画像1のキャプション)", text, re.DOTALL)
  ...
  # 画像2のプロンプトを取得
  prompt2_match = re.search(r"画像2のプロンプト：(.*?)(?:\n\n|画像2のキャプション)", text, re.DOTALL)
...
def writedrive(username, user_id, tweet_id, sentence,comment,p1,c1,p2,c2,kanzyolist,nitatext):
...
for i in range(1000):
  try:
    ...
    tweet_divs=search()
    tweetlist=[]
    for tweet_div in tweet_divs[1:]:
        id,name,twid=gettweetinfo(tweet_div)
        tweet_body = tweet_div.find("p", class_="Tweet_body__XtDoj")
        if "返信先" in tweet_body.text:
          print("・リプ")
          #print()
          continue
        if not contains_keywords(tweet_body.text):
          print("・入ってないよ")
          #print()
          continue
        print("(抽出情報→",id,name,twid,end="  ")
        print("(抽出ツイート→",tweet_body.text)
        tweetlist.append([twid,id,name,remove_urls(tweet_body.text)])
        #print()
    for t in tweetlist[:]:
        try:
            omoshiroi, fukin, roman, mukisitu, henjin, gensou, atama, nankaido, jiko, shiteki, kotobawari, kanseki, comment = tensuka(t[-1])
        except Exception as e:
        ...
        print(t[-1])
        hyoka=check_and_mark(t[-1], omoshiroi, fukin, roman, mukisitu, henjin, gensou, atama, nankaido, jiko, shiteki, kotobawari, kanseki)
        if hyoka=="〇":
          print("評価→",hyoka)
          prompt1, caption1, prompt2, caption2=ai_setumei_comment(t[-1])
          print("prompt1",prompt1)
          print("caption1",caption1)
          print("prompt2",prompt2)
          print("caption2",caption2)
          gazouseisei(prompt1,t[0],1)
          gazouseisei(prompt2,t[0],2)  
          #print()
          kanzyolist=kanzyo(t[-1])
          new_tweet_vector = vectorize_text(t[-1])
          save_vectors(file_path,t[0],t[-1],new_tweet_vector)
          vector_list.append(new_tweet_vector)
          tweet_data.append((t[0], t[-1]))
          ...
          writedrive(t[1],t[2],t[0],t[-1],comment,prompt1,caption1,prompt2,caption2,kanzyolist,nitatext)
        else:
          print("評価→",hyoka)
        print("-----------------------")
        #writedrive(t[1],t[2],t[0],t[-1])
  #twid,id,name,remove_urls(tweet_body.text)
    ...
    print(i,"回目処理を完了　待機終了---------------------")
  except Exception as e:
    print(f"1Error processing tweet: {e}")
    print("contents",t[-1])
    print()
    sys.exit()